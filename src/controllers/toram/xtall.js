...
export const searchXtal = async (req, res) => {
  try {
    const { name } = req.query;
    if (!name) {
      const nonvalid = struckError(status.bad, "Masukan nama xtal", "-");
      ...
      return res
        ...
        .json(
          struckError(
            status.notFound,
            "xtal search",
            "xtal yang dicari tidak ditemukan",
          ),
        );
    }

    const result = struckRes(status.berhasil, "xtal ditemukan", data);
